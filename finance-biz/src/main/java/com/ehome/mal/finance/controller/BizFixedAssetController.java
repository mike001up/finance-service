...
public class BizFixedAssetController {
    ...
    @ApiOperation("删除固定资产")
    @DeleteMapping("/{id}")
    public R<Boolean> deleteAsset(@PathVariable Long id) {
        return R.ok(fixedAssetService.deleteAsset(id));
    }

    @ApiOperation("处置固定资产")
    @PostMapping("/dispose/{id}")
    public R<Boolean> disposeAsset(@PathVariable Long id) {
        return R.ok(fixedAssetService.disposeAsset(id));
    }
