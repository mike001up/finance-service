...
public class BizVoucherController {
    ...
    @ApiOperation("修改凭证")
    @PutMapping
    public R<Boolean> updateVoucher(@RequestBody BizVoucher voucher) {
        return R.ok(voucherService.updateVoucher(voucher));
    }

    @ApiOperation("删除凭证")
    @DeleteMapping("/{id}")
    public R<Boolean> deleteVoucher(@PathVariable Long id) {
        return R.ok(voucherService.deleteVoucher(id));
    }
